...
const translations = {
  ar: {
    ...
    verificationSent: "تم إرسال رابط التحقق! تحقق من بريدك.",
  },
  en: {
    home: "Home",
    reports: "Reports",
...
function Login() {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      // جلب البيانات الكاملة من السيرفر
      ...
      // التحقق إذا كان السبب عدم تأكيد البريد
      if (error.response?.status === 403 && error.response?.data?.code === "EMAIL_NOT_VERIFIED") {
        toast(
          (toastInstance) => (
            <div style={{ fontFamily: "Tajawal", textAlign: lang === "ar" ? "right" : "left" }}>
              <p style={{ margin: "0 0 10px", fontWeight: 700 }}>{t.emailNotVerified}</p>
              <button
                onClick={async () => {
                  toast.dismiss(toastInstance.id);
                  try {
                    await resendVerification(loginEmail.trim().toLowerCase());
                  ...
                  <div style={{ textAlign: "center", marginTop: 4 }}>
                    <button
                      ...
                      style={{
                        ...
                        cursor: "pointer", fontFamily: "Tajawal", fontWeight: 700,
